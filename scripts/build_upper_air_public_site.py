...
def _nco_latest_text(daily: pd.DataFrame) -> tuple[str, str]:
    ...
    if pd.isna(expected) or not expected:
        return f"Latest day: {received:.0f} of — expected", f"Complete through {date_text}; expected inventory unavailable"
    return f"Latest day: {received:.0f} of {expected:.0f} expected · {percent:.1f}%", f"Complete through {date_text}"
...
def _nco_heatmap_markup(
    ...
    for days in (7, 14, 30, 90):
        ...
        metric_cards.append(
            ...
            f'<strong>{current_text}</strong><small title="Compares with the immediately preceding equal-length period">{html.escape(format_pp_delta(delta))}</small></div>'
        )
    return (
...
if(ncoPayloadElement){{
  ...
  const ncoShowCell=date=>{{
    ...
    ncoDetail.innerHTML='<strong>'+ncoEscape(ncoFormatDate(date))+'</strong><span>'+ncoPct(row.percent)+' · '+Number(row.received).toFixed(0)+' of '+(Number.isFinite(Number(row.expected))?Number(row.expected).toFixed(0):'—')+' expected</span><span class="nco-cell-models">'+ncoEscape(sources.length?sources.join(' · '):'No applicable source record')+'</span>';
  }};
...
if(ncoPayloadElement){{
  ...
  const ncoDetailHtml=(date,row)=>{{
    ...
    return '<strong>'+ncoEscapeDetail(ncoFormatDateDetail(date))+'</strong><span>'+(Number.isFinite(Number(row.percent))?Number(row.percent).toFixed(1)+'%':'No data')+' · '+Number(row.received).toFixed(0)+' of '+(Number.isFinite(Number(row.expected))?Number(row.expected).toFixed(0):'—')+' expected</span><span class="nco-cell-models">'+ncoEscapeDetail(sources)+'</span>';
  }};
